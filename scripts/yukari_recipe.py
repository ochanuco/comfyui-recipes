...
SIZES = {"lounge": (1024, 1536), "portrait": (1024, 1024),
         ...
         "winded": (1152, 1152),
         # `winded`'s square, for the reason written there: a seated girl fits
         # neither the portrait frames nor the landscape ones. Asleep she is
         # more compact again -- knees together, arms in -- so the risk this
         # frame carries is the empty half of the canvas beside her, and that
         # is where a second figure gets drawn.
         "doze": (1152, 1152),
...
def positive(pose: str, costume: str = "default") -> str:
    ...
    if pose == "winded":
        ...
        face = _splice(face, ", looking at viewer", "")
    if pose == "doze":
        # `nape`'s reason reached through the eyes instead of the camera: they
        # are shut, so an instruction to look at the viewer has no referent and
        # either argues with the pose or opens them.
        #
        # `small mouth` STAYS, which is where this parts from `swelter` and
        # `winded`. Those two removed it because the mouth was the biggest
