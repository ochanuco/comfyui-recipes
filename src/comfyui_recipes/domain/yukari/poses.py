...
POSES = {
    ...
    # gone. `(from side:1.4)` makes the silhouette read as the exercise and
    # `(foreshortening:1.35)` gives the near thigh its mass. `(reaching:1.2)`
    # does not come up in weight or in words: the `leaning forward` /
    # `outstretched arms` pair drags the whole render into vivid saturation
    # that nothing downstream pulls back. No `from above` camera -- it draws
    # a top-down crouch.
    "reach": (
        "(solo:1.5), (sitting:1.5), (from side:1.4), (foreshortening:1.35), "
        "(legs together:1.35), (stretching:1.3), (reaching:1.2), "
